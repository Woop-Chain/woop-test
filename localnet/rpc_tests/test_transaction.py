...
def cross_shard_txs():
    ...
    """
    s0_test_tx = {
        "from": "woc1ue25q6jk0xk3dth4pxur9e742vcqfwulhwqh45",
        "to": "woc1t40su52axu207vgc6ymcmwe0xmml4njrskk2vf",
    ...
    s1_test_tx = {
        "from": "woc1t40su52axu207vgc6ymcmwe0xmml4njrskk2vf",
        "to": "woc1qljfd3pnfjwr86ll6d0s6khcqhw8969p9l7fw3",
...
def test_get_pool_stats():
    ...
    raw_response = base_request("wiki_getPoolStats", params=[], endpoint=endpoints[0])
    ...
    raw_response = base_request("wikiv2_getPoolStats", params=[], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_current_transaction_error_sink():
    ...
    """
    error_tx = {
        "from": "woc1ujsjs4mhds75xnws0yx0v8l2rvyp67arwzqrvz",
        "to": "woc1wfn43ynxhhdrrjnddqcr74u38frqc7hqjhhdkx",
    ...
    reference_response = [
        {
            ...
            "error-message": "transaction gas-price is 0.000000000000000000 WOC: transaction underpriced"
        }
    ]

    response = base_request('wiki_sendRawTransaction', params=[error_tx["signed-raw-tx"]],
    ...
    # Check v1
    raw_response = base_request("wiki_getCurrentTransactionErrorSink", params=[],
                                endpoint=endpoints[error_tx["from-shard"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("wikiv2_getCurrentTransactionErrorSink", params=[],
                                endpoint=endpoints[error_tx["from-shard"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_resend_cx(cross_shard_txs):
    ...
    for tx in cross_shard_txs:
        raw_response = base_request("wiki_resendCx", params=[tx["hash"]],
                                    endpoint=endpoints[tx["shardID"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)

        raw_response = base_request("wikiv2_resendCx", params=[tx["hash"]],
                                    endpoint=endpoints[tx["shardID"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
...
def test_get_pending_cx_receipts():
    ...
    """
    cx = {
        "from": "woc19l4hghvh40fyldxfznn0a3ss7d5gk0dmytdql4",
        "to": "woc1ds3fayprfl6j7yd6mpwfncj9c0ajmhvmvhnmpm",
        # erupt concert hat tree anger discover disease town gasp lemon gesture fiber spread season mixture host awake tennis issue orbit member film winter glass
        "amount": "1000",
        "from-shard": 0,
        "to-shard": 1,
    ...
    while time.time() - start_time <= tx_timeout * 2:  # Cross shards are generally slower...
        if not v1_success:
            raw_response = base_request("wiki_getPendingCXReceipts", endpoint=endpoints[cx["to-shard"]])
            response = check_and_unpack_rpc_response(raw_response, expect_error=False)
            assert_valid_json_structure(reference_response, response)
        ...
        if not v2_success:
            raw_response = base_request("wikiv2_getPendingCXReceipts", endpoint=endpoints[cx["to-shard"]])
            response = check_and_unpack_rpc_response(raw_response, expect_error=False)
            assert_valid_json_structure(reference_response, response)
            for cx_receipt in response:
                for r in cx_receipt["receipts"]:
...
def test_get_cx_receipt_by_hash_v1(cross_shard_txs):
    ...
    raw_response = base_request("wiki_getCXReceiptByHash", params=[cross_shard_txs[0]["hash"]],
...
def test_get_cx_receipt_by_hash_v2(cross_shard_txs):
    ...
    raw_response = base_request("wikiv2_getCXReceiptByHash", params=[cross_shard_txs[0]["hash"]],
                                endpoint=endpoints[cross_shard_txs[0]["toShardID"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_send_raw_transaction_v1():
    tx = {
        "from": "woc1pvkjamc0q96s6z62qzz6e09k2qrqqdj34ylxvd",
        "to": "woc1mjunf85vnhc4drv57ugsyg2fxjnsq920qzkpwq",
    ...
    response = base_request('wiki_sendRawTransaction', params=[tx["signed-raw-tx"]],
    ...
    while time.time() - start_time <= tx_timeout:
        if get_transaction(tx["hash"], tx["from-shard"]) is not None:
            raw_response = base_request('wiki_sendRawTransaction', params=[tx["signed-raw-tx"]],
                                        endpoint=endpoints[tx["from-shard"]])
...
def test_send_raw_transaction_v2():
    tx = {
        "from": "woc13lu674f3jkfk2qhsngfc2vhcf372wprctdjvgu",
        "to": "woc14jeshxg75gdr5dz8sg7fm2sjvw7snnsdw98f0y",
    ...
    # Submit new transaction...
    response = base_request('wikiv2_sendRawTransaction', params=[tx["signed-raw-tx"]],
                            endpoint=endpoints[tx["from-shard"]])
    tx_hash = check_and_unpack_rpc_response(response, expect_error=False)
    assert tx_hash == tx["hash"], f"Expect submitted transaction to get tx hash of {tx['hash']}, got {tx_hash}"
    ...
    while time.time() - start_time <= tx_timeout:
        if get_transaction(tx["hash"], tx["from-shard"]) is not None:
            raw_response = base_request('wikiv2_sendRawTransaction', params=[tx["signed-raw-tx"]],
                                        endpoint=endpoints[tx["from-shard"]])
            response = check_and_unpack_rpc_response(raw_response, expect_error=True)
            assert_valid_json_structure(reference_response, response)
            assert reference_response["code"] == response["code"], f"Expected error code {reference_response['code']}, " \
...
def test_get_transaction_by_hash_v1():
    ...
    raw_response = base_request("wiki_getTransactionByHash",
...
def test_get_transaction_by_hash_v2():
    ...
    raw_response = base_request("wikiv2_getTransactionByHash",
                                params=[init_tx_record["hash"]],
                                endpoint=endpoints[init_tx_record["from-shard"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_pending_transactions_v1():
    tx = {
        "from": "woc1twhzfc2wr4j5ka7gs9pmllpnrdyaskcl5lq8ye",
        "to": "woc13awvzpjt7n3hcrmxax3elps7a6vw46u63kc28p",
    ...
    start_time = time.time()
    while time.time() - start_time <= tx_timeout:
        raw_response = base_request("wiki_pendingTransactions", endpoint=endpoints[tx["from-shard"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
...
def test_pending_transactions_v2():
    tx = {
        "from": "woc1u57rlv5q82deja6ew2l9hdy7ag3dwnw57x8s9t",
        "to": "woc1zchjhmsxksamlxuv7h3k9h5aeury670n2jck2u",
    ...
    start_time = time.time()
    while time.time() - start_time <= tx_timeout:
        raw_response = base_request("wikiv2_pendingTransactions", endpoint=endpoints[tx["from-shard"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        for pending_tx in response:
            if pending_tx["hash"] == tx["hash"]:
...
def test_get_transaction_by_block_hash_and_index_v1():
    ...
    raw_response = base_request("wiki_getTransactionByBlockHashAndIndex",
...
def test_get_transaction_by_block_hash_and_index_v2():
    ...
    raw_response = base_request("wikiv2_getTransactionByBlockHashAndIndex",
...
def test_get_transaction_by_block_number_and_index_v1():
    ...
    raw_response = base_request("wiki_getTransactionByBlockNumberAndIndex",
...
def test_get_transaction_by_block_number_and_index_v2():
    ...
    raw_response = base_request("wikiv2_getTransactionByBlockNumberAndIndex",
