...
woop_dir="$(go env GOPATH)/src/github.com/woop-chain/woop"
...
function kill_localnet() {
  pushd "$(pwd)"
  cd "$woop_dir" && bash ./test/kill_node.sh
  popd
}

function setup() {
  if [ ! -d "$woop_dir" ]; then
    echo "Test setup FAILED: Missing woop directory at $woop_dir"
    exit 1
  fi
...
function build_and_start_localnet() {
  local localnet_log="$woop_dir/localnet_deploy.log"
  rm -rf "$woop_dir/tmp_log*"
  rm -rf "$woop_dir/.dht*"
  rm -f "$localnet_log"
  rm -f "$woop_dir/*.rlp"
  pushd "$(pwd)"
...
function wait_for_localnet_boot() {
  ...
  until $valid; do
    result=$(curl --silent --location --request POST "localhost:9500" \
      --header "Content-Type: application/json" \
      --data '{"jsonrpc":"2.0","method":"wiki_blockNumber","params":[],"id":1}' | jq -r '.result')
    if ((result>0)); then
      valid=true
    else 
...
function wait_for_epoch() {
  ...
  until ((cur_epoch >= "$1")); do
    ...
    --header "Content-Type: application/json" \
    --data '{"jsonrpc":"2.0","method":"wikiv2_latestHeader","params":[],"id":1}' | jq .result.epoch)
    if ((i > timeout)); then
      echo "TIMEOUT REACHED"
      exit 1
    fi
